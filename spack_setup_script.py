...
"""
    ...
    if not cluster:
        print("No system specified -- exiting.")
        sys.exit(1)
    ...
    if not cluster_dir.is_dir():
        print("No packages for system -- exiting.")
        sys.exit(1)
    ...
    if not install_location.is_dir():
        print(f"Making folder for {cluster} external packages")
    ...
    if target_file.is_file():
        ...
        if hash1 != hash2:
            print("Current packages file does not match repo, will replace")
            need_new_file = True
    else:
        need_new_file = True
    ...
    if need_new_file:
        print(f"Installing {install_file} to {install_location}")
    ...
    if spack_repo_setup:
        ...
        subprocess.run([spack_exe, "repo", "remove", "cupecs"], stderr=subprocess.DEVNULL)
        subprocess.run([spack_exe, "repo", "remove", "compass"], stderr=subprocess.DEVNULL)
        
        subprocess.run([spack_exe, "repo", "add", str(cwd / "spack_pkgs" / "spack_repo" / "cupecs")], check=True)
        subprocess.run([spack_exe, "repo", "add", str(cwd / "spack_pkgs" / "spack_repo" / "compass")], check=True)
